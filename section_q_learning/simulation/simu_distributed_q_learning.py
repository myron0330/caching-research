...
from section_q_learning.display.tools import display_dict_, display_distributed_
...
def algorithm_comparison(algorithms, comparison_algorithm=None,
    ...
    for algorithm in algorithms:
        ...
        elif algorithm.func_name == 'distributed_q_learning':
            algorithm_type = 'distributed_q_learning'
        else:
...
def plot_distributed_comparison():
    file_name = filter(lambda x: x.startswith('algorithm.rewards.distributed_q_learning') and x.endswith('20-20-100-20-0.9.pk'),
                       listdir('../performance'))[0]
    rewards = pickle.load(open('../performance/{}'.format(file_name), 'r+'))
    frame = pd.DataFrame(rewards)
    rewards_data = {column: list(frame[column]) for column in frame.columns}
    parameters = {
        'display_length': 100,
        'line_width': 2.5,
        'title_size': 20,
        ...
        'texts': [
            ...
            {
                'args': (-3.3, 12.5, '${R_{b,t}(S_{b, t-1}, C_{b,t}|\\Theta_{b,t})}$'),
                'kwargs': {
                    'horizontalalignment': 'center',
                    'verticalalignment': 'center',
                    'fontsize': 20,
        ...
        'save_path': '../plots/distributed_q_learning.jpg',
    }
    display_distributed_(rewards_data, **parameters)
...
if __name__ == '__main__':
    ...
    # t_algorithms = [distributed_q_learning]
    ...
    #                      circles=50, dump=True, with_lfu=False, with_lru=False, **plot_parameters)
