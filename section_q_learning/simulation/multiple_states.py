...
def plot_markov_state():
    config = '../cfg/algorithm_comparison.cfg'
    agent = Agent.from_(cfg_file=config)
    state_list = [agent.variables.states[0]]
    state_index, seed = 0, 0
    while len(state_list) < 1000:
        current_state_index = state_index
        state_index, state = agent.generate_markov_random_states(current_state_index=current_state_index,
                                                                 seed=seed)
        seed += 1
        state_list.append(state)

    state_dict = OrderedDict([(state, list()) for state in sorted(agent.variables.states)])
    for _, state in enumerate(state_list):
        other_state = filter(lambda x: x != state, state_dict)[0]
        if _ == 0:
            state_dict[state].append(1)
            state_dict[other_state].append(0)
        else:
            state_dict[state].append(state_dict[state][-1] * (_ / (_ + 1)) + 1 / (_ + 1))
            state_dict[other_state].append(state_dict[other_state][-1] * (_ / (_ + 1)))

    state_dict = {'state={}'.format(key): value for key, value in state_dict.iteritems()}
    ...
    parameters = {
        ...
        'texts': [
            ...
            {
                'args': (-60, 0.5, '$\\overline{p}_{state, 1:t}$'),
                'kwargs': {
                    'horizontalalignment': 'center',
                    'verticalalignment': 'center',
                    'fontsize': 20,
        ...
        'save_path': '../plots/markov_state.jpg',
    }
    # display_list_(state_list, **parameters)
    display_dict_(state_dict, **parameters)
...
if __name__ == '__main__':
    ...
    #                      circles=1000, dump=False, with_lfu=False, with_lru=False, **plot_parameters)
    plot_markov_state()
